...
As discussed further in \S~\ref{sec:MIRB}, in one case, (C22, \pid{13972}), this visit was re-purposed to verify a change to the MIRB\tacq{IMAGE} configuration required due to the increasing background (see \S~\ref{sec:MIRB}).}
	...
	\item{The third orbit of each program takes back-to-back BOA$\times$MIRA and BOA$\times$MIRB \tacq{IMAGE}s~and target (HIP66578) TIME-TAG images (with lamp flashes).
	As in the second orbit, a second BOA$\times$MIRA \tacq{IMAGE} is then performed to provide a second measurement of the offset.
