...
\colhead{\textit{NAME}}&\colhead{\textit{MODE}}&\colhead{}&
\colhead{(s)}&\colhead{Lamp\#$\times$Current}&
\colhead{$\times$\textit{OPT\_ELEM}}&
\colhead{\textit{XPOS\tablenotemark{d}}}&\colhead{\textit{YPOS\tablenotemark{c}}}&\colhead{}\\
...
\bottomrule
\enddata
\vspace{-0.2 cm}\tablenotetext{a}{The intended PtNe lamp flashes of \pid{13124} did not occur as expected. This was corrected in all other programs.}
\vspace{-0.1 cm}\tablenotetext{b}{\textit{APERYPOS}, the AD aperture mechanism positions, are stored in the FSW in \textsc{pcmech\_ApMDispPosition}.
...
\vspace{-0.1 cm}\tablenotetext{c}{It is not uncommon that the XD aperture location (\textit{APERXPOS}) is $\pm 1$ step off from its nominal position.
...
\vspace{-0.2 cm}
\tablecomments{PSA \textsc{EXPTYPE}=\extsci{} exposures contain simultaneous target and PtNe lamp (WCA) time-tag (TT) images.}
\end{deluxetable}
\end{center}
...
\colhead{(s)}&\colhead{Lamp\#$\times$Current}&
\colhead{$\times$\textit{OPT\_ELEM}}&
\colhead{\textit{XPOS\tablenotemark{b}}}&\colhead{\textit{YPOS\tablenotemark{c}}}&\colhead{}\\
\colhead{(1)}&\colhead{(2)}	&	\colhead{(3)}&\colhead{(4)} &
\colhead{(5)}&\colhead{(6)}	&	\colhead{(7)}&\colhead{(8)} &
\colhead{(9)}&\colhead{(10)}	&	\colhead{(11)}
}
...
\bottomrule
\enddata
\tablenotetext{a}{The intended PtNe (WCA) lamp flashes of \pid{13124} did not occur as expected. This was corrected in all other programs.}
...
\tablenotetext{b}{\textit{APERYPOS}, the AD aperture mechanism positions, are stored in the FSW in \textsc{pcmech\_ApMDispPosition}.
The trailing "0.1" reported in the FITS headers is a conversion anomaly that is present in all aperture positions (\textit{APERXPOS} \& \textit{APERYPOS}).}
\tablenotetext{c}{It is not uncommon that the XD aperture location (\textit{APERXPOS}) is $\pm1$ step off from its nominal position.
The XD aperture mechanism positions are stored in the FSW in \textsc{pcmech\_ApMXDispPosition} (see Table~\ref{tab:ApMXDispPosition}).}
\tablecomments{PSA \textsc{EXPTYPE}=\extsci{} exposures contain simultaneous target and PtNe lamp (WCA) time-tag (TT) images.}
\end{deluxetable}
\end{center}
...
\colhead{\textit{OSID}}&\colhead{\textit{NAME}}&
%\colhead{\textit{NAME}}&
%\colhead{\textit{MODE\tablenotemark{b}}}&\colhead{}&
\colhead{(s)}&\colhead{Lamp\#$\times$Current}&
\colhead{$\times$\textit{OPT\_ELEM}}&
\colhead{\textit{XPOS\tablenotemark{a}}}&\colhead{\textit{YPOS\tablenotemark{b}}}&\colhead{}\\
\colhead{(1)}&\colhead{(2)}	&	\colhead{(3)}&\colhead{(4)} &
\colhead{(5)}&\colhead{(6)}	&	\colhead{(7)}&\colhead{(8)}
}
\startdata
...
\tablenotetext{a}{\textit{APERYPOS}, the AD aperture mechanism positions are stored in the FSW in \textsc{pcmech\_ApMDispPosition}.
...
\tablenotetext{b}{It is not uncommon that the XD aperture location (\textit{APERXPOS}) is $\pm1$ step off from its nominal position.}
