...
\bottomrule
\enddata
\tablenotetext{a}{Starting on 2009.200 with \pr{63095}; some early ERO and calibrations used slightly different subarrays.}
\tablenotetext{b}{Starting early in LP1 on Aug 27, 2009 (2009.2239) with \pr{63378}.}
...
\tablenotetext{f}{Updated Feb. 20, 2017 (2017.051) for LP4 (\pr{86945}). Both FUVA and FUVB are used for all LP4 \tacq{PEAKXD}s (\textit{NUM\_POS$>1$}) TA exposures. These subarrays also avoid the FUVB ``Hot-Spot''.}
...
\end{deluxetable}
\subsubsection{Trimming of COS FUV TA subarrays due to FUVB ``Hot-Spot''.}\label{subsec:hotspots}
...
In FUVB detector coordinates, the location of the hot-spot is at [X$_{DET}$,Y$_{DET}$]$\simeq$[14895,482].
